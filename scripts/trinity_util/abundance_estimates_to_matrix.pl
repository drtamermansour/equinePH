...
main: {
    ...
    if ($cross_sample_fpkm_norm =~ /^TMM$/i) {
        my $cmd = "./run_TMM_scale_matrix.pl --matrix $fpkm_matrix_file > $out_prefix.$cross_sample_fpkm_norm.fpkm.matrix";
        &process_cmd($cmd);
    }
    elsif ($cross_sample_fpkm_norm =~ /^UpperQuartile$/) {
        my $cmd = "./run_UpperQuartileNormalization_matrix.pl --matrix $fpkm_matrix_file > $out_prefix.$cross_sample_fpkm_norm.fpkm.matrix";
        &process_cmd($cmd);
    }
